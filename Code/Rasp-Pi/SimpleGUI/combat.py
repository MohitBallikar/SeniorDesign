    ...
    for mcu_id in range(4):
        for row in range(3):
            for col in range(5):
                piece = board.grids[mcu_id][row][col]
                if piece:
                    player_id, piece_type, num_troops = piece.split('_')
                    player_id = int(player_id)
                    num_troops = int(num_troops)

                    # Check if the piece is in the safe space of any enemy
                    if col == 0 and player_id != safe_spaces[mcu_id]['first']:
                        enemy_id = safe_spaces[mcu_id]['first']
                        players[enemy_id]['hp'] -= num_troops
                        print(f"Player {enemy_id} loses {num_troops} HP due to Player {player_id}'s {piece_type} in safe space.")
                    elif col == 4 and player_id != safe_spaces[mcu_id]['last']:
                        enemy_id = safe_spaces[mcu_id]['last']
                        players[enemy_id]['hp'] -= num_troops
                        print(f"Player {enemy_id} loses {num_troops} HP due to Player {player_id}'s {piece_type} in safe space.")
