# simple_gui.py
# hours of consciousness in past 5 days: 108
# %age conscious: 90%
# horses exploded: 612,413
# shadows smiling: 823
# big bags of money handed over: 2
# 8:15pm Wednesday, July 15, 2024
...
board = Board()
players = {
    1: {'hp': 20, 'gold': 0, 'rubies': 0, 'sapphires': 0, 'buildings': [], 'buildings_with_hp_bonus': [], 'attack_cards': 0, 'growth_cards': 0},
    2: {'hp': 20, 'gold': 0, 'rubies': 0, 'sapphires': 0, 'buildings': [], 'buildings_with_hp_bonus': [], 'attack_cards': 0, 'growth_cards': 0},
...
def prompt_play_card(player_id):
    layout = [[sg.Text(f"Play a card.")],
              [sg.Button("OK")]]
    window = sg.Window("Play a Card.",layout, size=(800,480), finalize=True)
            ...
            if result.returncode == 0:
                ...
                if players[player_id]['attack_cards'] > 0:
                    if cut == "'0x83'":
                        ...
                        sg.popup("You have hired mercenaries.\n +5 troops to all existing units.")
                        players[player_id]['attack_cards'] -= 1
                        played = True
                        break
                    elif cut == "'0xb3'":
                        fireball(player_id)
                        sg.popup("You have cast Breaching Inferno.\n -10 hp to all other players.")
                        players[player_id]['attack_cards'] -= 1
                        played = True
                        break
                if players[player_id]['growth_cards'] > 0:
                    if cut == "'0xd3'":
                        ...
                        sg.popup("You have keenly fortified your base.\n +10 hp to your base.")
                        players[player_id]['growth_cards'] -= 1
                        played = True
                        break
                    elif cut == "'0x53'":
                        bounty(player_id)
                        sg.popup("Rejoice! You have found a wealth of minerals.\n +8 of every resource.")
                        players[player_id]['growth_cards'] -= 1
                        played = True
                        break
    window.close()
...
def prompt_place_piece(player_id, piece_type):
    layout = [[sg.Text(f"Place your {piece_type}")],
              [sg.Button("OK")]]
    window = sg.Window("Place Piece", layout, size=(800,480), finalize=True)
...
def start_turn_layout(player_id):
    return [[sg.Text(f"Player {player_id}, Begin Turn")],
            [sg.Text(f"HP: {players[player_id]['hp']}")],
            [sg.Text(f"Gold: {players[player_id]['gold']}")],
            [sg.Text(f"Rubies: {players[player_id]['rubies']}")],
            [sg.Text(f"Sapphires: {players[player_id]['sapphires']}")],
...
def building_purchase_layout(player_id):
    ...
    if affordable_buildings:
        layout = [[sg.Text("Choose a building to purchase")],
                  [sg.Text(f"HP: {players[player_id]['hp']}")],
                  [sg.Text(f"Gold: {players[player_id]['gold']}")],
                  [sg.Text(f"Rubies: {players[player_id]['rubies']}")],
                  [sg.Text(f"Sapphires: {players[player_id]['sapphires']}")]] + [[sg.Button(b)] for b in affordable_buildings]
    else:
...
def card_purchase_layout(player_id):
    ...
    if affordable_cards:
        layout = [[sg.Text("Purchase a card or continue")],
                  ...
                  [sg.Text(f"Gold: {players[player_id]['gold']}")],
                  [sg.Text(f"Rubies: {players[player_id]['rubies']}")],
                  [sg.Text(f"Sapphires: {players[player_id]['sapphires']}")]] + [[sg.Button(card)] for card in affordable_cards]
    else:
...
def display_siege_breakdown(siege_results):
    ...
    layout = [[[[sg.Text(f"Player {result['attacker_player_id']}'s {result['piece_type']} with {result['num_troops']} troops has sieged Player {result['defender_player_id']}.\n"
    ...
    window = sg.Window("Siege Report", layout)
    event, _= window.read()
    window.close()
...
def display_attack_breakdown(attack_results):
    ...
    layout = [[[[sg.Text(f"Player {result['attacker_player_id']}'s {result['attacker_type']} has attacked Player {result['defender_player_id']}'s {result['defender_type']}. "
    ...
    layout.append([sg.Button("OK")])
    window = sg.Window("Combat Report", layout)
    event, _=window.read()
    window.close()
    

def attack_layout(player_pieces):
    layout = [[sg.Text("Select a piece to move")]] + [[sg.Button(f"{piece[0]} at ({piece[3]}, {piece[4]})")] for piece in player_pieces] + [[sg.Button("End Attack")]]
    return layout
...
def game_loop():
    ...
    while True:
        ...
        window = sg.Window(f"Player {player_id}'s Turn", layout, size=(800,480))
        event, _ = window.read()
        window.close()
        ...
        if players[player_id]['attack_cards'] > 0 or players[player_id]['growth_cards']> 0:
            ...
            window = sg.Window("Card Play", layout, size=(800,480))
            event, _ = window.read()
            window.close()
            if event == sg.WIN_CLOSED:
                break
        ...
        window = sg.Window("Decision", layout, size=(800,480))
        event, _ = window.read()
        window.close()
        if event == sg.WIN_CLOSED:
            break
        if event == "Development":
            ...
            window = sg.Window("Growth", layout, size=(800,480))
            event, _ = window.read()
            ...
            window = sg.Window("Building Purchase", layout, size=(800,480))
            event, _ = window.read()
            ...
            window = sg.Window("Card Purchase", layout, size=(800,480))
            event, _ = window.read()
            ...
            window = sg.Window("Attack", layout, size=(800,400))
            event, _ = window.read()
            selected_piece = None
        ...
        window = sg.Window("End Turn", layout, size=(800,480))
        event, _ = window.read()
        window.close()
